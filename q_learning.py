...
def compute_cost(target_q, a_t, q_value):
    ...
    Args:
        ...
        a_t: current action
        q_value: current Q-value of each possible action
    Returns:
        cost
    """
    # TO DO: Q-value for the action.
    state_q = tf.reduce_sum(tf.multiply(a_t, q_value), axis=1)
    # TO DO: Q-Learning Cost.
    cost = tf.reduce_mean(tf.square(state_q - target_q))
    return cost
...
def compute_target_q(next_state_batch, r_batch, readout_j1_batch, minibatch):
    ...
    for i in range(len(minibatch)):
        if minibatch[i][4]:     # terminal state
            target_q = r_batch[i]
        else:
            target_q = r_batch[i] + GAMMA * np.max(readout_j1_batch[i])
...
def trainNetwork(s, readout, sess):
    ...
    while True:
        ...
        for i in range(0, K):   # K=5, select an action every 5th frame
